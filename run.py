...
from Src.API.clicka import clicka
...
# All'interno della funzione stream, dopo gli altri provider
# Add Eurostreaming
if provider_maps['EUROSTREAMING'] == "1" and ES == "1":
    url_eurostreaming = await eurostreaming(id, client)
    if url_eurostreaming:
        print(f"Eurostreaming Found Results for {id}")
        streams['streams'].append({'name': f"{Name}", 'title': f'{Icon}Eurostreaming', 'url': url_eurostreaming, 'behaviorHints': {'bingeGroup': 'eurostreaming'}})

# Add Clicka
if provider_maps['CLICKA'] == "1" and CC == "1":
    url_clicka = await clicka(id, client)
    if url_clicka:
        print(f"Clicka Found Results for {id}")
